...
export default function WeeklyTaskProgress(props) {
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.stick_container}>
        {props.data.map((x, y) => (
          <CandleStick
            key={y}
            day={x.day}
            completed={x.completed / highest}
            total={x.total / highest}
          />
        ))}
      </div>
      ...
      <p style={{ top: "35px" }} className={styles.label}>
        {highest}
      </p>
      <p style={{ top: "75px" }} className={styles.label}>
        {Math.round(highest / 2)}
      </p>
      <p style={{ top: "110px" }} className={styles.label}>
        0
      </p>
    </div>
  );
  ...
  return (
    <div className={styles.stick}>
      <div className={styles.stick_total_container}>
        <div
          style={{ height: `${props.total * 80}px ` }}
          className={styles.total}
        >
          <div
            style={{ height: `${props.completed * 80}px ` }}
